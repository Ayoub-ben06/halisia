...
const nav = [
  ...
  { label: "Screening", icon: ShieldCheck, href: "/screening" },
  { label: "Zakat", icon: Coins, href: "/zakat" },
];
...
export default async function DashboardV1Page() {
  ...
  const totalInvested =
    ...
    crypto.reduce((sum, asset) => sum + Number(asset.total_invested), 0) +
    gold.reduce((sum, asset) => sum + Number(asset.total_invested), 0);
  ...
  const complianceScore = totalValue
    ...
    : 0;
  const firstName =
    user.email?.split("@")[0].split(/[._-]/)[0] ?? "Investisseur";

  return (
    <div className="min-h-screen bg-[#0c0f0d] font-sans text-[#e2e3df]">
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-72 flex-col border-r border-[#4d4637]/20 bg-[#1a1c1a] md:flex">
        <div className="flex h-20 items-center gap-3 px-8">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#c9a84c] text-[#111412]">
            <WalletCards className="h-5 w-5" />
          </div>
          <span className="text-xl font-bold">Halal Invest FR</span>
        </div>
        <nav className="flex-1 space-y-1 px-6 py-6">
          {nav.map(({ label, icon: Icon, active, href }) => (
            <Link
              key={label}
              href={href ?? "/dashboard_v1"}
              className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold ${active ? "bg-[#333533] text-[#e6c364]" : "text-[#d0c5b2] hover:bg-[#282b28]"}`}
            >
              <Icon className="h-5 w-5" />
              {label}
            </Link>
          ))}
          <p className="px-4 pb-3 pt-8 text-[11px] font-bold uppercase tracking-[0.15em] text-[#706957]">
            Gestion
          </p>
          <Link
            href="/portfolio"
            className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm text-[#d0c5b2] hover:bg-[#282b28]"
          >
            <WalletCards className="h-5 w-5" />
            Portefeuille
          </Link>
        ...
        <div className="m-6 flex items-center gap-3 rounded-2xl bg-[#282b28] p-4">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#c9a84c]/20 font-bold text-[#e6c364]">
            {firstName.charAt(0).toUpperCase()}
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-bold capitalize">{firstName}</p>
            <p className="text-[11px] text-[#d0c5b2]">Membre Premium</p>
          </div>
          <LogOut className="h-4 w-4 text-[#d0c5b2]" />
        </div>
      </aside>

      <div className="md:ml-72">
        <header className="sticky top-0 z-40 flex min-h-20 items-center gap-5 bg-[#111412]/90 px-4 backdrop-blur-md sm:px-8">
          ...
          <div className="ml-auto flex w-full max-w-xl items-center gap-3">
            ...
            <button className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#1e201e]">
        ...
        <main className="mx-auto max-w-[1400px] space-y-8 p-4 sm:p-8">
          ...
          <div className="grid gap-8 lg:grid-cols-12">
            <div className="space-y-8 lg:col-span-8">
              ...
              <section className="overflow-hidden rounded-[2rem] border border-[#4d4637]/20 bg-[#1e201e]">
                  ...
                  <table className="w-full min-w-[680px]">
                    <thead className="bg-[#282b28]/50 text-left text-[10px] uppercase tracking-widest text-[#d0c5b2]">
                      <tr>
                        ...
                        <th className="px-8 py-4 text-right">Perf.</th>
                      </tr>
                    </thead>
            ...
            <aside className="space-y-6 lg:col-span-4">
              <section className="rounded-[2rem] border border-[#4d4637]/20 bg-[#1e201e] p-6">
...
function Kpi({
  ...
  return (
    <div className="rounded-[2rem] border border-[#4d4637]/20 bg-[#1e201e] p-8">
