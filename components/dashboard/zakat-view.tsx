...
/** `illicitRatio` is a percentage; `purificationAmount` = estimated dividends × impure revenue ratio (EUR). */
export type ZakatAsset = { id: string; name: string; category: "Actions conformes" | "Actions à vérifier" | "ETF islamique" | "Or" | "Bitcoin" | "Crypto" | "Cash"; value: number; status: "compliant" | "debated" | "non_compliant" | "unknown"; purchaseDate: string; illicitRatio: number; purificationAmount: number };
export type ZakatPayment = { id: string; paid_at: string; amount: number; currency: string };
...
export function ZakatView({ assets, nisab, nisabBasis = "gold", goldPricePerGram, silverPricePerGram, initialPaymentDate, initialPayments = [], isAuthenticated, displayCurrency = "EUR", displayRate = 1 }: { assets: ZakatAsset[]; nisab: number; nisabBasis?: "gold" | "silver"; goldPricePerGram: number; silverPricePerGram: number; initialPaymentDate: string; initialPayments?: ZakatPayment[]; isAuthenticated: boolean; displayCurrency?: string; displayRate?: number }) {
  ...
  const yearPayments = payments.filter((payment) => payment.paid_at.startsWith(String(year)));
  const paid = isAuthenticated ? yearPayments.length > 0 : guestPaid;
  ...
  useEffect(() => { if (!isAuthenticated) setGuestPaid(localStorage.getItem(`zakat-paid-${year}`) === "true"); }, [year, isAuthenticated]);
  const calculation = useMemo(() => {
    ...
    if (wizardValues) { addCategory("Cash", wizardValues.cash + (wizardValues.includeIncome ? wizardValues.salary + wizardValues.otherIncome : 0)); addCategory("Or", wizardValues.goldGrams * goldPricePerGram); addCategory("Cash", wizardValues.silverGrams * silverPricePerGram); }
    ...
    const purificationRows = eligible.filter((asset) => asset.purificationAmount > 0).map((asset) => ({ ...asset, amount: asset.purificationAmount }));
  ...
  }, [assets, includeBitcoin, nisab, wizardValues, year, goldPricePerGram, silverPricePerGram]);
  const total = calculation.zakat + calculation.purification;
  async function markPaid() {
    if (!isAuthenticated) { const next = !guestPaid; setGuestPaid(next); localStorage.setItem(`zakat-paid-${year}`, String(next)); return; }
    ...
    try {
      ...
      if (!user) return;
      if (paid) {
        const { error } = await supabase.from("zakat_payments").delete().in("id", yearPayments.map((payment) => payment.id));
        if (!error) setPayments((current) => current.filter((payment) => !payment.paid_at.startsWith(String(year))));
      } else {
        const paidAt = year === currentYear ? new Date().toISOString().slice(0, 10) : `${year}-12-31`;
        const { data, error } = await supabase.from("zakat_payments").insert({ user_id: user.id, paid_at: paidAt, amount: total, currency: "EUR" }).select("id, paid_at, amount, currency").single();
        if (!error && data) setPayments((current) => [data as ZakatPayment, ...current]);
      }
    } finally {
  ...
  return <div className="mx-auto max-w-[1280px] p-4 text-[#ffffff] sm:p-8">
    <header className="text-center"><span className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-primary"><Sparkles className="h-3.5 w-3.5" />Méthode inspirée des standards AAOIFI · Résultat en direct</span><h1 className="mt-5 text-3xl font-bold">Calculez votre Zakat {year}</h1><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-[#a9a291]">Retrouvez automatiquement votre assiette zakatable, votre purification et le suivi du Hawl à partir de votre portefeuille.</p><label className="relative mt-6 inline-block"><span className="sr-only">Année de référence</span><select value={year} onChange={(event) => setYear(Number(event.target.value))} className="h-11 appearance-none rounded-lg border border-[#344038] bg-[#1a1c1a] pl-4 pr-10 text-sm outline-none focus:border-[#c9a84c]">{[currentYear, currentYear - 1, currentYear - 2].map((item) => <option key={item} value={item}>Année de référence : {item}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-3.5 h-4 w-4 text-[#a9a291]" /></label></header>
    <section className="mt-8 grid gap-5 lg:grid-cols-3"><SummaryCard icon={Coins} label="Zakat à verser" value={euro.format(calculation.zakat)} badge={calculation.reached ? "Nisab atteint" : "Sous le nisab"} tone="gold"><button type="button" onClick={() => setWizardOpen(true)} className="mt-5 inline-flex h-10 w-full items-center justify-center gap-2 rounded-md border border-[#9a8035] text-sm text-[#e6c364] hover:bg-[#c9a84c]/10"><Scale className="h-4 w-4" />Modifier ou voir le calcul</button></SummaryCard><SummaryCard icon={Droplets} label="Purification totale" value={euro.format(calculation.purification)} badge={calculation.purification > 0 ? "Action requise" : "À jour"} tone="orange" /><SummaryCard icon={HandCoins} label="Total à donner" value={euro.format(total)} badge={paid ? "Versement enregistré" : undefined} tone="light"><button type="button" onClick={() => void markPaid()} disabled={savingPayment} className={`mt-5 inline-flex h-10 w-full items-center justify-center gap-2 rounded-md border text-sm disabled:opacity-60 ${paid ? "border-emerald-500/50 text-halal-compliant" : "border-[#9a8035] text-[#e6c364] hover:bg-[#c9a84c]/10"}`}><CheckCircle2 className="h-4 w-4" />{paid ? `Versé${yearPayments[0] ? ` le ${new Date(yearPayments[0].paid_at).toLocaleDateString("fr-FR")}` : ""} — annuler` : "Marquer comme versé"}</button></SummaryCard></section>
    <section id="detail-zakat" className="mt-10"><Title icon={Info}>Détail du calcul Zakat</Title><div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-t-xl border border-white/[0.05] border-l-[#c9a84c] bg-[#1a1c1a] px-5 py-4 text-xs"><span><strong>Nisab {year} :</strong> {euro.format(nisab)} <span className="text-[#8f8878]">({nisabBasis === "silver" ? "595 g d’argent — école hanafite" : "85 g d’or"})</span>　 <strong>Patrimoine :</strong> {euro.format(calculation.zakatable)}</span><span className={calculation.reached ? "flex items-center gap-2 text-halal-compliant" : "flex items-center gap-2 text-halal-debated"}>{calculation.reached ? <CheckCircle2 className="h-4 w-4" /> : <TriangleAlert className="h-4 w-4" />}{calculation.reached ? "Vous dépassez le nisab" : "Nisab non atteint"}</span></div><div className="overflow-x-auto rounded-b-xl border border-t-0 border-white/[0.05]"><table className="w-full min-w-[700px] text-sm"><thead className="bg-[#1a1c1a] text-left text-[11px] uppercase tracking-wider text-[rgba(255,255,255,0.6)]"><tr><th className="px-4 py-4">Catégorie</th><th className="px-4 py-4 text-right">Valeur</th><th className="px-4 py-4 text-center">Zakatable</th><th className="px-4 py-4 text-right">Zakat (2,5 %)</th></tr></thead><tbody>{calculation.categories.map((item) => { const Icon = categoryIcons[item.category]; const allowed = item.status !== "non_compliant"; return <tr key={item.category} className="border-t border-white/[0.05] bg-[#1a1c1a] hover:bg-[#1c211d]"><td className="px-4 py-4"><span className="flex items-center gap-2"><Icon className="h-4 w-4 text-[#c9a84c]" />{item.category}</span></td><td className="px-4 py-4 text-right">{euro.format(item.value)}</td><td className={`px-4 py-4 text-center ${allowed ? item.status === "debated" ? "text-halal-debated" : "text-halal-compliant" : "text-[#8f8878]"}`}>{allowed ? item.status === "debated" ? "Optionnel" : "Oui" : "Non (purification)"}</td><td className="px-4 py-4 text-right text-[#e6c364]">{euro.format(calculation.reached && allowed ? item.value * 0.025 : 0)}</td></tr>; })}{!calculation.categories.length && <tr><td colSpan={4} className="px-4 py-10 text-center text-[#8f8878]">Aucun actif pour cette année.</td></tr>}</tbody></table></div>
      ...
      <div className="mt-5 rounded-xl border border-white/[0.05] border-l-2 border-l-[#c9a84c] bg-[#1a1c1a] p-5 sm:p-6">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          ...
          {isAuthenticated ? (
            ...
            <a href="/login?next=%2Fzakat" className="inline-flex h-11 shrink-0 items-center justify-center rounded-lg bg-[#c9a84c] px-5 text-sm font-bold text-[#111412] hover:bg-[#e6c364]">Se connecter pour ajouter une date</a>
          )}
        </div>
      </div>
    </section>
    <section className="mt-10"><Title icon={Droplets}>Détail de la purification</Title><p className="mt-2 text-xs text-[#a9a291]">Part des dividendes reçus correspondant aux revenus non conformes de l’entreprise (dividendes estimés × ratio de revenus impurs). Les plus-values ne sont pas concernées.</p><div className="mt-5 overflow-x-auto rounded-xl border border-white/[0.05]"><table className="w-full min-w-[720px] text-sm"><thead className="bg-[#1a1c1a] text-left text-[11px] uppercase tracking-wider text-[rgba(255,255,255,0.6)]"><tr><th className="px-4 py-4">Actif</th><th className="px-4 py-4 text-right">Valeur position</th><th className="px-4 py-4 text-right">Revenus non conformes</th><th className="px-4 py-4 text-right">Montant à purifier</th><th className="px-4 py-4 text-right">Statut</th></tr></thead><tbody>{calculation.purificationRows.map((asset) => <tr key={asset.id} className="border-t border-white/[0.05] bg-[#1a1c1a]"><td className="px-4 py-4 font-medium">{asset.name}</td><td className="px-4 py-4 text-right">{euro.format(asset.value)}</td><td className="px-4 py-4 text-right text-halal-debated">{asset.illicitRatio.toFixed(2).replace(".", ",")} %</td><td className="px-4 py-4 text-right">{euro.format(asset.amount)}</td><td className="px-4 py-4 text-right"><span className="inline-flex items-center gap-1 rounded-full border border-orange-500/40 bg-orange-500/10 px-2.5 py-1 text-xs text-halal-debated"><ShieldAlert className="h-3 w-3" />À purifier</span></td></tr>)}{!calculation.purificationRows.length && <tr><td colSpan={5} className="px-4 py-10 text-center text-[#8f8878]">Aucun revenu non conforme identifié.</td></tr>}</tbody><tfoot><tr className="border-t border-white/[0.05] bg-[#1a1c1a]"><td colSpan={3} className="px-4 py-4 text-right font-semibold">Total purification :</td><td className="px-4 py-4 text-right font-bold text-halal-debated">{euro.format(calculation.purification)}</td><td /></tr></tfoot></table></div></section>
...
const zakatFaqs = [
  ["C’est quoi le Nisab ?", "Le Nisab est le seuil minimal de patrimoine à partir duquel la Zakat devient obligatoire. Halisia utilise par défaut la valeur de 85 grammes d’or ; l’école hanafite retient 595 grammes d’argent, sélectionnable dans vos préférences."],
